...
//   int j = 0;
  for(int id : nearest)
  {
    if (!processed[id])
    {
      clusterHelper(id, points, cluster, processed, tree, distanceTol, maxSize);
    }
  }
}
template<typename PointT>
...
int main ()
{
    ...
    for (int i=0; i<cloud->points.size(); i++) 
  	...
  	std::vector<int> nearby = tree->search({3,30},1.0);
  	...
  	//
  	std::vector<std::vector<int>> clusters = euclideanCluster(cloud->points, tree, 3.0);
  	//
	...
	std::vector<Color> colors = {Color(1,0,0), Color(0,1,0), Color(0,0,1)};
  	for(std::vector<int> cluster : clusters)
  	{
  		...
  		for(int indice: cluster)
  			clusterCloud->points.push_back(pcl::PointXYZ(cloud->points[indice][0],cloud->points[indice][1],0));
